...
.namespace Configuration {
    .label MIN_SPRITE_Y = 34
    .label TopSpriteIrqRasteLine = MIN_SPRITE_Y - 12 // why 12? runtime of the top sprite irq part?

    .label ScreenRamAddress = $0400
    .label MAX_SPRITES = 24
    .label MAX_SPRITE_Y = 220

    // those three ZP registers are used to store and restore the current register values in the sprite IRQ
    .label spriteIrqAccuSaveZp = allocateSpecificZpByte($2)
    .label spriteIrqXRegisterSaveZp = allocateSpecificZpByte($3)
    .label spriteIrqYRegisterSaveZp = allocateSpecificZpByte($4)
    
    .label firstSortSprZP = allocateZpByte()    // this is either 0 or MAX_SPRITES. It's used to store the start index into the sorted 
                                                // double buffers (see sorted* buffers for x,y,pointers and colors)
    
    .label newFrame = allocateZpByte()
    
    .label sprOrder = allocateZpBytes(MAX_SPRITES + 1, "sprOrder")  // this is a pointer into the sprite data to enable only swapping two bytes when 
                                                        // swapping sprites during sort

    .label sprY = allocateZpBytes(MAX_SPRITES + 1, "sprY")
...
main:
    jsr initSpriteMultiplexerData
    jsr initSprites
    jsr mainSpriteLoop.UpdateFrame
    jsr initInterrupts
    lda #$ff
...
initSpriteMultiplexerData: {                         // Initialize the sprite multiplexing system
    .var tempZp = allocateZpByte()

    lda #$0                                         // set first sprite in the beginning
...
spriteLoop:
    txa
    sta Configuration.sprOrder,x

    lda #$ff
    sta Configuration.sprY,x                    // init all Y coordinates with $ff to skip them by default

    cpx #Configuration.MAX_SPRITES              // we initialize an additional byte at the end of the sprOrder and sprY array
    ...
    lda tempZp                                  // get current bit mask
    ...
    asl tempZp                                  // shift bitmask one bit to the left
    bne bitmaskStillOk                          // this is the negated bitmask, so there will be a 1, not 0 as stored in tempZp
...
initSprites: {
    ...
    lda #$0a
    sta $d025
    lda #$00//ff                        //Set all sprites multicolor
    sta $d01c
...
mainSpriteLoop:{
    .var tempSpriteIndex = allocateZpByte()
    .var tempPreviousOrder = allocateZpByte()
    .var tempD010 = allocateZpByte()
    .var sortedSpriteEndIndex = allocateZpByte()
    .var temp8 = allocateZpByte()
...
SSpr_Loop1:     
    ldy Configuration.sprOrder,x                    // check for coordinates being in order (old y <= new y for all ys)
    cmp Configuration.sprY,y                        // a=0 in first iteration, otherwise a=previous sprite y
    ...
    lda Configuration.sprY,y                        // load the new y corrdinate into accumulator
    ldy Configuration.sprOrder-1,x                  // load previous order into y register
    sty Configuration.sprOrder,x                    // and store it as new order. This pair has to be swapped at least.
...
SSpr_Swap1:                             
    ldy Configuration.sprOrder-1,x                  // load previous order into y register
    sty Configuration.sprOrder,x                    // and store it as new order.
    cmp Configuration.sprY,y                        // compare against the previous y
...
SSpr_SwapDone1: 
    ...
    sty Configuration.sprOrder,x
    ldx tempSpriteIndex
    ldy Configuration.sprOrder,x

SSpr_NoSwap1:   
    lda Configuration.sprY,y
SSpr_NoSwap2:   
    inx                                             // next sprite 
    ...
    lda #<Configuration.sprOrder                // get ZP address of sprite order array start
    sec
    sbc Configuration.firstSortSprZP            // subtract 0 or MAX_SPRITES (buffer area 1 or 2)
    sta copyLoopSourceAddress
    ldy Configuration.firstSortSprZP            // use the index also for y indexing to stay within sprOrder array
                                                // the subtract needs to happen because y is used in the copy loop to index into both
                                                // a double buffered array and the single buffered temp zp array (sprY, spriteXLo, ...)
...
SSpr_CopyLoop1: 
    ...
    // x register is used to index into the sprite data arrays (sprY, spriteXLo, spriteXHi, spriteColors, spritePointers)
    // y register is used to index into the sorted arrays (first or second half of double buffers when writing)
    ldx copyLoopSourceAddress: Configuration.sprOrder,y         // load sprite index (start will be modified before)
                                                                // because y is used to index into the sorted arrays, the base address has been reduced 
                                                                // by the offset (0 or MAX_SPRITES) before to be able to also use y as index into the sprOrder array
    lda Configuration.sprY,x                                    // load y coord of this sprite. ($ff when invisible)    
...
SSpr_CopyLoop2: 
    ldx copyLoopSourceAddress2: Configuration.sprOrder,y
    lda Configuration.sprY,x
...
SSpr_CopyLoop2Done:
    ...
    .var tempIrqSpriteIndex = allocateZpByte()
...
UF_NotMoreThan8:tax
                lda d015Tbl,x
UF_NoSprites2:  sta topSpriteIrq.d015Value
                beq UF_NoSprites
                ldx Configuration.firstSortSprZP                //Find out sprite Y-range for the fastloader
                stx continuationSpriteIrq.firstSortedSpriteIndex
                //lda sortedSpritesY,x                  //(where to avoid the timed data transfer)
                //sec
                //sbc #$04
                //sta Irq1_MinSprY+1
                //lda sortSprY-1,y
