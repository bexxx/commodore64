...
interruptHandler:
    ...
    lda #<secondRasterInterrupt                 // (2 cycles) setup 2nd raster interrupt handler
    ...
    tsx                                         // (2 cycles) save stack pointer into code of
    stx secondRasterInterrupt.stackpointerValue // (4 cycles) 2nd handler to return to correct location
    nop                                         // (2 cycles)
    nop                                         // (2 cycles)
    nop                                         // (2 cycles)
    lda #VIC.ENABLE_RASTER_INTERRUPT_MASK       // (2 cycles) ack raster interrupt to enable 2nd
                                                // total
                                                // 26 cycles
    ...
    inc VIC.CURRENT_RASTERLINE_REG              // (6 cycles) 2. IRQ in der übernächsten Zeile (22)!!!
...
finalRasterInterruptStart:
    ...
    //*** Bad Line!         
    sty $d020                                   //  4 cycles
    ldy #$0d                                    //  2 cycles light green
    //bit $01                                   //  3 cycles
    nop                                         //  2 cycles
    sty $D020                                   //  4 cycles set color
    nop                                         //  2 cycles
    nop                                         //  2 cycles
    nop                                         //  2 cycles
    nop                                         //  2 cycles
